    ...
    loadRoute: function (store) {
        if ('chart' === this.lookupReference('reportTypeField').getValue()) {
            ...
            for (store = 0; store < deviceIds.length; store++) {
                ...
                chartSeries.push({ type: 'line', store: deviceStore, yField: this.chartType, xField: 'fixTime', highlightCfg: { scaling: Traccar.Style.chartMarkerHighlightScaling }, colors: [Traccar.app.getReportColor(deviceIds[store])],
...
Ext.define('Traccar.view.Report', {
    ...
    { xtype: 'cartesian', itemId: 'chart', plugins: { ptype: 'chartitemevents', moveEvents: !0 }, store: 'ReportRoute', axes: [{ title: Strings.reportChart, type: 'numeric', position: 'left' }, { type: 'time', position: 'bottom', fields: ['fixTime'], label:{rotate:{degrees:-45}} }], listeners: { itemclick: 'onChartMarkerClick' }, insetPadding: Traccar.Style.chartPadding }
    ]
});
