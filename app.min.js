...
Ext.define('Traccar.AttributeFormatter', {
    ...
    speedFormatter: function (value) { return Ext.getStore('SpeedUnits').formatValue(value, Traccar.app.getAttributePreference('speedUnit')) },
...
Ext.define('Traccar.model.Event', {
    extend: Ext.data.Model,
    identifier: 'negative',
    fields: [{ name: 'id', type: 'int' }, { name: 'type', type: 'string' }, { name: 'serverTime', type: 'date', dateFormat: 'c' }, { name: 'deviceId', type: 'int' }, { name: 'positionId', type: 'int' }, { name: 'geofenceId', type: 'int' }, { name: 'maintenanceId', type: 'int' }, {
        name: 'text',
...
Ext.define('Traccar.view.ReportController', {
    ...
    updateButtons: function () {
        ...
        this.lookupReference('animationButton').setDisabled(disabled);
    ...
    loadRoute: function (store) {
        if ('chart' === this.lookupReference('reportTypeField').getValue()) {
            ...
            for (store = 0; store < deviceIds.length; store++) {
                ...
                chartSeries.push({
                    type: 'line', store: deviceStore, yField: this.chartType, xField: 'fixTime', highlightCfg: { scaling: Traccar.Style.chartMarkerHighlightScaling }, colors: [Traccar.app.getReportColor(deviceIds[store])],
    ...
    eventsColumns: [{ text: Strings.positionFixTime, dataIndex: 'serverTime', xtype: 'datecolumn', renderer: Traccar.AttributeFormatter.getFormatter('serverTime') }, { text: Strings.reportDeviceName, dataIndex: 'deviceId', renderer: Traccar.AttributeFormatter.getFormatter('deviceId') },
...
Ext.define('Traccar.view.Report', {
    ...
    tbar: {
        ...
        }, { text: Strings.reportExport, reference: 'exportButton', disabled: !0, handler: 'onReportClick' }, { text: Strings.reportEmail, reference: 'emailButton', disabled: !0, handler: 'onReportClick' }, {text: 'Start Animation', reference: 'animationButton', disabled: !0, handler: 'onAnimationClick'}, { text: Strings.reportClear, handler: 'onClearClick' }]
    },
    layout: 'card',
...
Ext.define('Traccar.view.map.BaseMap', {
    ...
    initMap: function () {
        ...
        var esriVectorSource = new ol.source.Vector({
            loader: function (extent, resolution, projection) {
                ...
                if (window.location.hostname == "192.168.100.96") {
                    userHostName = "http://192.168.100.50";
                }
                else {
        ...
        var wmsSource_Parsa_ATMs = new ol.source.TileWMS({
            url: 'http://192.168.100.50:6080/arcgis/rest/services/ParsaAtms/MapServer/WMSServer',
            params: { 'LAYERS': '0' },
                        ...
                        items: [
                            {
                                ...
                                store: {
                                    ...
                                    data: [
                                        { moon: 'ماه قبل', successtr: feature.get('Lag1MonthSucess').toLocaleString(), failtr: feature.get('Lag1MonthFail').toLocaleString(), sumtr: feature.get('Lag1MonthTotal').toLocaleString() },
                                        { moon: 'دو ماه قبل', successtr: feature.get('Lag2MonthSucess').toLocaleString(), failtr: feature.get('Lag2MonthFail').toLocaleString(), sumtr: feature.get('Lag2MonthTotal').toLocaleString() },
                                        { moon: 'سه ماه قبل', successtr: feature.get('Lag3MonthSucess').toLocaleString(), failtr: feature.get('Lag3MonthFail').toLocaleString(), sumtr: feature.get('Lag3MonthTotal').toLocaleString() },
                                    ],
                                },
                            },
                            {
                                ...
                                store: {
                                    ...
                                    data: [
                                        { month: "سه ماه قبل", success: feature.get('Lag3MonthSucess'), fail: feature.get('Lag3MonthFail') },
                                        { month: "دو ماه قبل", success: feature.get('Lag2MonthSucess'), fail: feature.get('Lag2MonthFail') },
                                        { month: "ماه قبل", success: feature.get('Lag1MonthSucess'), fail: feature.get('Lag1MonthFail') },
                                    ]
                                },
...
Ext.define('Traccar.view.map.MapMarkerController', {
    ...
    loadReport: function (store, data) {
        if (data) {
            ...
            var routeCoords = this.reportRoute[this.reportRoute.length - 1].getGeometry().getCoordinates();
            var routeLength = routeCoords.length;
            var routeFeature = new ol.Feature({
                type: 'route',
                geometry: this.reportRoute[this.reportRoute.length - 1].getGeometry()
            });
            var styles = {
            ...
            //   });
              ...
              var animationvectorLayer = new ol.layer.Vector({
              ...
              this.getView().getMap().addLayer(animationvectorLayer);
        }
    },
...
Ext.define('Traccar.view.map.MapController', {
    ...
    onFollowClick: function (button, pressed) { pressed && this.selectedMarker && this.getView().getMapView().setCenter(this.selectedMarker.getGeometry().getCoordinates()) },
...
Ext.define('Traccar.controller.Root', {
    ...
    zoomToAllDevices: function () {
        var lat = Traccar.app.getPreference('latitude',
            0);
